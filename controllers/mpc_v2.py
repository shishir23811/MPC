...
class ModelPredictiveController:
    ...
    def _reference_input(self, x_ref, x_ref_next):
        ...
        if v_ref <= 1e-3:
            ...
            # Reference psi is unwrapped (continuous), so take the raw difference.
            # wrap_angle() would be wrong here as it would fold a smooth multi-turn
            # heading into (-pi, pi) and produce incorrect psi_dot estimates.
            dpsi = psi_ref_next - psi_ref
    ...
    def solve(self, x0, t0, u_prev=None):
        ...
        # Align the vehicle's current yaw to the reference yaw branch so that
        # the linearisation error term (x[i,2] - psi_ref) is small.  The
        # reference psi from figure_eight() is unwrapped and may lie outside
        # (-pi, pi), so we shift x0's yaw by the nearest 2*pi multiple.
        psi_ref0 = x_ref[0][2]
        psi0_aligned = x0[2] + round((psi_ref0 - x0[2]) / (2.0 * np.pi)) * (2.0 * np.pi)
        x0_opt = x0.copy()
        x0_opt[2] = psi0_aligned

        x = cp.Variable((self.horizon + 1, 4))
        u = cp.Variable((self.horizon, 2))

        cost = 0.0
        constraints = [x[0, :] == x0_opt]

        for i in range(self.horizon):
            A_i, B_i, c_i = self._linearize(x_ref[i], u_ref[i])
            constraints += [x[i + 1, :] == A_i @ x[i, :] + B_i @ u[i, :] + c_i]

            # Wrap yaw error into (-pi, pi) so cost is never inflated by 2*pi
            # offsets between the (possibly multi-revolution) reference and the
            # vehicle state.  We do this by adjusting the reference psi to the
            # branch nearest the previous reference psi.
            x_ref_i = x_ref[i].copy()
            if i > 0:
                prev_psi_ref = x_ref[i - 1][2]
                x_ref_i[2] = x_ref_i[2]  # already continuous from unwrapped reference
